...
"""
online device
    Shimadzu 2 data & StandBy delay 
    plot --> mean over frames as an raw in an image; raws of the image sorted by value of StandBy delay  

    combines data from 2 bridges
    - works on Slave interface for both Shimadzu 2 & delay channel --> waits for interface HPVX (not good!) --> needs change
Parameters
----------
None
...
-------
plot
    updating image raw pixels as mean of frames, columns according to delay values 
"""
...
interfaceSlave = 'tcp://10.253.0.52:54333' # KOHZU bridge ... delays & Shimadzu 2
...
class imageView(QtGui.QMainWindow):
    ...
    def updatePlot(self, dataHPVX, dataSlave):
            ...
            if dataSlave is not None:  
                delay = dataSlave[device_name['shimadzu2del']][device_prop['delayCam']]*1.

                if (device_name['shimadzu2'] in dataSlave.keys() ): 
                    imageData1 = np.array(dataSlave[device_name['shimadzu2']][device_prop['camera']]*1.)     # *1. makes a copy 
                    if len(imageData1.shape)==3:
                        cam1Buff = np.mean(imageData1,axis=(1,2))
                    # else:
                    #     cam1Buff = np.mean(imageData1,axis=(1)) # live mode -- bullshit, but does not crash            
            
            if delay is not None and cam1Buff is not None:
                self.updateDF(delay,cam,cam1Buff)   ################################3
                # calculate
