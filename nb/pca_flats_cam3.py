...
TODO 
    - .
usage: pca_flats_cam2.py int int int int [-rk int] [-pdir str]
...
cam_dark0 = run_dark.get_array('SPB_EHD_HPVX2_3/CAM/CAMERA:daqOutput','data.image.pixels')
cam_dark = cam_dark0.values[:,2:,:,:]   # w/o first two frames
tid_dark, buf_dark, x, y = cam_dark.shape
print('Shape of Dark: ', tid_dark, buf_dark, x, y)
mean_dark = np.mean(cam_dark,axis=(0,1)).reshape(-1)

# read flat-fields
file_flat = RunDirectory(file_flat)  # adjust proposal number
data_flat0 = file_flat.get_array('SPB_EHD_HPVX2_3/CAM/CAMERA:daqOutput','data.image.pixels')
data_flat = data_flat0.values[:,2:,:,:]   # w/o first two frames
tid_flat, buf_flat, x, y = data_flat.shape
print('Shape of Flat-Fields: ', tid_flat, buf_flat, x, y)
data_flat = data_flat.reshape(tid_flat * buf_flat, x*y)
...
if path[-1] != '/':
    path+='/'
filename = path + "pca_info_cam3" + "_flat_run" + str(run_number_flat) + "_rank" + str(n_components) + ".h5"
